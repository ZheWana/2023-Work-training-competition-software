...
uint8_t _data_rotation[4] = { ST7735_MADCTL_MX, ST7735_MADCTL_MY, ST7735_MADCTL_MV, ST7735_MADCTL_RGB };
#endif

#if defined(ST7735_1_44_DEFAULT_ORIENTATION) || defined(ST7735_MINI_DEFAULT_ORIENTATION)
uint8_t _data_rotation[4] = {ST7735_MADCTL_MX, ST7735_MADCTL_MY, ST7735_MADCTL_MV, ST7735_MADCTL_BGR};
#endif

uint8_t _value_rotation = 0;
int16_t _height = ST7735_HEIGHT, _width = ST7735_WIDTH;
uint8_t _xstart = ST7735_XSTART, _ystart = ST7735_YSTART;
...
void ST7735_WriteChar_Fast(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    for (i = 0; i < font.height; i++) {
        ...
        for (j = 0; j < font.width; j++) {
            if ((b << j) & 0x8000) {
                Font_Image[cnt++] = color >> 8;
                Font_Image[cnt++] = color & 0xFF;
            } else {
                Font_Image[cnt++] = bgcolor >> 8;
                Font_Image[cnt++] = bgcolor & 0xFF;
            }
        }
    }
    ST7735_DrawImage(x, y, font.width, font.height, (uint16_t *) Font_Image);
}
...
void ST7735_DrawString_Fast(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    while (*str) {
        ...
        ST7735_WriteChar_Fast(x, y, *str, font, color, bgcolor);
        x += font.width;
        str++;
    }
    TFT_CS_H();
...
void ST7735_FillRectangle_Fast(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    int32_t buff_size = sizeof(uint16_t) * w * h;
    TFT_CS_L();
    TFT_DC_D();
    while (buff_size > 0) {
        HAL_SPI_Transmit(&ST7735_SPI_PORT, data, chunk_size, 0xFF);
...
void ST7735_FillScreen_Fast(uint16_t color) {
    ST7735_FillRectangle_Fast(0, 0, _width, _height, color);
}
