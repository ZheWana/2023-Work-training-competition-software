...
//    PID_Init(&CarInfo.msPid[0], 10, 0.5f, 0);//0, 0.2, 0,
//    PID_Init(&CarInfo.mpPid[0], 0.15f, 0, 0);// 0.033, 0, 0,
        ...
        if (cnt == period) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (CarInfo.mPsiCtr) {
                    ...
                    float *aimPtr = &CarInfo.msPid[i].ctr.aim;
                    ...
                    // Limit speed max
                    if (CarInfo.msPid[i].ctr.aim > CarInfo.spdLimit)CarInfo.msPid[i].ctr.aim = CarInfo.spdLimit;
                    if (CarInfo.msPid[i].ctr.aim < -CarInfo.spdLimit)CarInfo.msPid[i].ctr.aim = -CarInfo.spdLimit;
                }
