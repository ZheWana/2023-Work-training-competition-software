...
//        while (1)printf("PMW Init Error!\n");
//    if (QMC5883_Init() != 0)
//        while (1)printf("QMC Init Error!\n");
//    if ((res = ICM42605_Init()) != 0)
//        while (1)printf("ICM Init Error:%d\n", res);

    // Soft Init
    for (int i = 0; i < 4; i++) {
        PID_Init(&CarInfo.mPid[i], 10, 0.5f, 0);//0, 0.2, 0,
        PID_Init(&CarInfo.pPid[i], 0.15f, 0, 0);// 0.033, 0, 0,
    }
//    PID_Init(&CarInfo.mPid[0], 10, 0.5f, 0);//0, 0.2, 0,
//    PID_Init(&CarInfo.pPid[0], 0.15f, 0, 0);// 0.033, 0, 0,
...
//        CarInfo.curX += CarInfo.pmw.deltaX;
//        CarInfo.curY += CarInfo.pmw.deltaY;
//
//        // Get compass
//        QMC5883_GetData(&CarInfo.hmc);
//        printf("data:%f,%f,%f\n", CarInfo.hmc.Mx, CarInfo.hmc.My,
//               ToDig(atan2f(CarInfo.hmc.Mx, CarInfo.hmc.My)));
//
//        // Get gyro
...
//        printf("data:%f,%f,%f\n", CarInfo.icm.ax, CarInfo.icm.ay, CarInfo.icm.az);
        ...
        if (cnt == period) {
            ...
            for (int16_t i = 0; i < 4; i++) {
                ...
                int16_t res = 0;
                if (i == 1 || i == 3)
                    CarInfo.spd[i] = -1 * ((int16_t) (TIMx->CNT) - preCNT[i]);
                else
                    CarInfo.spd[i] = (int16_t) (TIMx->CNT) - preCNT[i];
                preCNT[i] = (int16_t) (TIMx->CNT);
            }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (CarInfo.psiCtr) {
                    ...
                    float *aimPtr = &CarInfo.mPid[i].ctr.aim;
                    float res = PID_Realize(&CarInfo.pPid[i], CarInfo.psi[i]);

//                    // Limit accelerate
                    if (res > *aimPtr && res > 0 && *aimPtr >= 0)
                        *aimPtr = *aimPtr < res ? *aimPtr + CarInfo.spdStep : *aimPtr;
                    else if (res < *aimPtr && res < 0 && *aimPtr <= 0)
                        *aimPtr = *aimPtr > res ? *aimPtr - CarInfo.spdStep : *aimPtr;
                    else *aimPtr = res;

                    // Limit speed max
                    if (CarInfo.mPid[i].ctr.aim > CarInfo.spdLimit)CarInfo.mPid[i].ctr.aim = CarInfo.spdLimit;
                    if (CarInfo.mPid[i].ctr.aim < -CarInfo.spdLimit)CarInfo.mPid[i].ctr.aim = -CarInfo.spdLimit;
                }

                // Speed loop
                float duty = PID_Realize(&CarInfo.mPid[i], CarInfo.spd[i]);
                if (duty < 0) {
                    duty = -duty;
                    dir = 0;
                }
